...
#        self.message_handler(F.text == 'Нет', self.handle_decline)
    ...
    async def start_command_access_rights(self, message: types.Message):
        ...
        participant = session.query(
            Participant
        ).filter_by(tg_id=str(user_id)).first()
...
#        """Обработчик отказа от дисклеймера"""
#        await message.answer(
#            text="Жаль, что вы отказались от участия."
#                 " Вы всегда можете нажать /start для повторного запуска."
#        )
